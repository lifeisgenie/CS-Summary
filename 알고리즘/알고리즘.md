...
- **조합론**
    - **순열 (Permutation)**
        - **설명**: 주어진 집합의 원소들을 나열하는 경우의 수를 계산한다. 순서가 중요하다.
        - **수식**: P(n,r)=(n−r)!n!
        - **시간 복잡도**: O(r) (단일 계산 기준)
        - **사용 예시**:
            - 여행 경로를 나열할 때 (TSP 문제)
    ...
    - **조합 (Combination)**
        - **설명**: 주어진 집합의 원소들 중에서 순서와 상관없이 뽑는 경우의 수를 계산한다.
        - **수식**: C(n,r)=r!(n−r)!n!
        - **시간 복잡도**: O(r) (단일 계산 기준)
        - **사용 예시**:
            - 복권 번호 조합 계산
...
- **확률 기반 알고리즘**
    - **몬테카를로 시뮬레이션 (Monte Carlo Simulation)**
        - **설명**: 확률론적 접근 방식으로 무작위 샘플을 생성하여 복잡한 문제를 근사적으로 해결.
        - **시간 복잡도**: O(n) (샘플 수에 비례)
        - **사용 예시**:
            - 원주율 계산
    ...
    - **랜덤화 알고리즘 (Randomized Algorithms)**
        - **설명**: 무작위성을 활용하여 문제 해결 과정을 최적화하거나 예측 불가능성을 부여.
        - **시간 복잡도**: 알고리즘에 따라 다름 (e.g., O(logn) 또는 O(n))
        - **사용 예시**:
            - 퀵정렬의 피벗 선택
...
- **슬라이딩 윈도우 (Sliding Window)**
    - **설명**: 고정 크기 또는 가변 크기의 윈도우를 사용하여 배열이나 문자열의 특정 구간을 효율적으로 탐색한다.
    - **시간 복잡도**: O(n)
    - **사용 예시**:
        - 최대/최소 부분합 계산.
        ...
        - 문자열에서 고유 문자 집합 길이 찾기.
- **분할-정복적 접근법**
    - **설명**: 문제를 작은 부분으로 나누고 각각 해결 후 결과를 합쳐서 전체 문제를 해결.
    - **시간 복잡도**: O(nlogn) (대표적으로 병합정렬)
    - **사용 예시**:
        - 병합 정렬, 퀵 정렬
...
- **유니온-파인드 (Union-Find, Disjoint Set)**
    - **설명**: 여러 집합을 효율적으로 관리하며 두 요소가 같은 집합에 속하는지 확인하거나 두 집합을 합친다.
    - **시간 복잡도**: O(α(n)) (α는 아커만 함수의 역함수)
    - **사용 예시**:
        - 그래프에서 연결성 확인 (최소 스패닝 트리).
        - 네트워크 연결 문제.
    ...
    int main() {
        ...
        printf("2 and 3 connected? %s\n", find(2) == find(3) ? "Yes" : "No");
...
- **두 포인터 기법 (Two-pointer Technique)**
    - **설명**: 두 개의 포인터를 사용해 배열이나 리스트를 효율적으로 탐색.
    - **시간 복잡도**: O(n)
        
        O(n)O(n)
        
    - **사용 예시**:
        - 정렬된 배열에서 특정 합을 만족하는 두 숫자 찾기
...
- **스위핑 기법 (Sweep Line Algorithm)**
    - **설명**: 정렬된 데이터를 선형으로 탐색하여 특정 조건을 만족하는 구간을 찾거나 문제를 해결.
    - **시간 복잡도**: O(nlogn)
    - **사용 예시**:
        - 구간 겹침 확인
        ...
        int main() {
            ...
            for (int i = 0; i < n - 1; i++) {
                if (intervals[i].end > intervals[i + 1].start) {
                    printf("Overlap found between [%d, %d] and [%d, %d]\n",
...
- **시간 복잡도 (Time Complexity)**
    - 알고리즘이 실행되는데 걸리는 시간의 상한선을 나타낸다. 입력 크기 n에 따라 알고리즘의 성능을 측정하고, 해당 알고리즘이 입력 크기가 커질 때 얼마나 빨리 실행되는지 또는 느려지는지를 평가하는 중요한 지표
    - **Big-O** (O): 알고리즘의 최악의 경우에 대한 상한선. 즉, 최악의 입력에 대해 얼마나 시간이 걸리는지를 나타낸다.
        - **예시)** 병합 정렬에서 **O(n log n)**: 병합 정렬은 입력 데이터가 어떤 형태이든 평균적으로나 최악의 경우에 O(nlog⁡n)의 시간이 걸린다.
    - **Omega** (Ω): 알고리즘의 최선의 경우에 대한 하한선. 즉, 최선의 입력에서 얼마나 시간이 걸리는지를 나타낸다.
        - **예시)** 선형 탐색에서 **Ω(n)**: 선형 탐색은 최소한 n번의 비교가 필요하므로 최선의 경우에도 O(n)의 시간이 걸린다.
    - **Theta** (Θ): 알고리즘의 평균적인 실행 시간을 낸다. 평균적인 입력에서 실행 시간이 어떻게 되는지에 대한 표기법이다.
        - **예시)** 그래프 탐색에서 **Θ(V + E)**: DFS나 BFS의 평균 시간 복잡도는 각 노드와 간선의 수에 비례한다.
    - **시간 복잡도 분석 기법**
        - **연산 횟수 세기**: 중첩된 루프에서 각 반복의 실행 횟수를 계산
        - **최대 반복 확인**: 입력 크기 n이 증가할 때 반복 횟수의 추세를 확인
        - **재귀 분석**: 재귀적 알고리즘은 점화식으로 변환 후 해석 (예: 마스터 정리 사용)
- **공간 복잡도 (Space Complexity)**
    - **설명**: 알고리즘이 실행되는데 필요한 추가 메모리의 양. 입력 크기 n에 따라 결정
    - **구성 요소**:
        - **고정 공간**: 알고리즘에서 사용하는 상수 크기의 메모리로, 입력 크기와 상관없이 일정한 양의 메모리를 사용하는 부분
            - **예시)** 변수, 상수
        - **가변 공간**: 입력 크기에 따라 변하는 메모리
            - **예시)** 동적 배열, 재귀 호출 스택
    - **예시)** 병합 정렬
...
- **최선, 평균, 최악의 경우 분석**
    - **설명**: 알고리즘의 성능을 평가할 때 입력 데이터에 따라 성능이 어떻게 달라지는지 분석하는 과정
        - **최선의 경우 (Best Case)**
            - **정의**: 알고리즘이 실행되는 동안 가장 적은 연산을 필요로 하는 경우이다.
            - **설명**: 최선의 경우는 데이터가 가장 이상적인 형태일 때 발생한다. 이 경우 알고리즘이 불필요한 연산을 피하거나 최소한으로만 연산을 진행한다.
        - **평균적인 경우 (Average Case)**
            - **정의**: 알고리즘의 실행 시간이나 성능을 측정할 때, 평균적인 입력값을 사용할 경우의 실행 시간을 의미한다.
            - **설명**: 알고리즘이 실제로 동작하는 대부분의 경우를 분석한다. 이 경우는 알고리즘이 정상적으로 작동할 때 나타날 수 있는 평균적인 성능을 나타내며, 모든 입력에 대해 가능한 연산을 평균 내어서 계산한다.
        - **최악의 경우 (Worst Case)**
            - **정의**: 알고리즘이 실행되는데 가장 많은 연산을 필요로 하는 경우이다.
            - **설명**: 최악의 경우는 입력 데이터가 알고리즘의 성능을 최대한 저하시킬 때 발생한다. 이는 알고리즘이 가장 비효율적으로 작동하는 시나리오로, 주어진 문제에 대해 최악의 입력을 고려하여 알고리즘의 한계를 평가한다.
    - **예시)**
        - **퀵 정렬:**
            - **최선의 경우**: 배열이 이미 정렬되어 있거나, 피벗이 배열의 중앙값에 가까운 경우
            - **평균적인 경우**: 배열이 랜덤하게 섞여 있을 때
            - **최악의 경우**: 배열이 이미 정렬되어 있거나 역순일 때 (불균형적인 분할)
        - **버블 정렬**:
            - **최선의 경우**: 배열이 이미 정렬되어 있는 경우 (교환이 발생하지 않음)
            - **평균적인 경우**: 배열이 랜덤하게 섞여 있을 때
            - **최악의 경우**: 배열이 완전히 역순으로 정렬된 경우
